# The red-black tree implemented in rb_tree.py has been modified by me for my
# purposes from the one in the MIT-licensed "Algos" repository found at
# https://github.com/stanislavkozlovski/algos.
# This repository has implementations for a large number of data structures,
# but I only needed a red-black tree.
# As is required by the MIT license, the original license is included in this
# directory. In addition, however, I wanted to link to the repository
# (as done above) and credit the creators.
# The project maintainers are listed in their repository's README as being
# Avi Aryan (https://github.com/aviaryan)
# and Pratyush Singh (https://github.com/singhpratyush).
# They credit the idea of the repository to
# Monal5031 (https://github.com/Monal5031).
...
class Node:
    ...
    # I hate how ugly this is, but it's the easiest way to get this working right now.
    def swapWith(self, other):
        for s in self.subscribers:
            s.node = other
        for s in other.subscribers:
            s.node = self
        self.subscribers, other.subscribers = other.subscribers, self.subscribers
...
class RedBlackTree:
    ...
    def add(self, value):
        ...
        if node_dir is None:
            return  parent # value is in the tree
        ...
        self.count += 1
        return self.find_node(value) #new_node
    ...
    def remove(self, value):
        ...
        if node_to_remove is None:  # node is not in the tree
            return
        self.removeGivenNode(node_to_remove)

    def removeGivenNode(self, node_to_remove):
        if node_to_remove.get_children_count() == 2:
            ...
            # HAD TO MAKE BIG CHANGES HERE!
            
            node_to_remove.value = successor.value  # switch the value

            node_to_remove.swapWith(successor)
...
#%%
# While I have also made minor changes to the original Red Black Tree
# above, most of my changes are the new methods I've added below this comment.

    def maximum(self, node):
        currNode = node
        while currNode.right != self.NIL_LEAF:
            currNode = currNode.right
        return currNode
    
    def minimum(self, node):
        currNode = node
        while currNode.left != self.NIL_LEAF:
    ...
    def successor(self, node):
        ...
        currNode = node
        # While we haven't hit the root and we're still to the right of the parent...
        while currNode.parent and currNode == currNode.parent.right:
    ...
    def predecessor(self, node):
        ...
        currNode = node
        # While we haven't hit the root and we're still to the left of the parent...
        while currNode.parent and currNode == currNode.parent.left:
            currNode = currNode.parent
        return currNode.parent
    
    def valueList(self):
        return self._valueList(self.root)
    
    def _valueList(self, node):
        ...
        if node.left != self.NIL_LEAF:
            if node.left.parent != node:
                print("Left parent problem here!")
            leftList = self._valueList(node.left)
        if node.right != self.NIL_LEAF:
            if node.right.parent != node:
                print("Right parent problem here!")
            rightList = self._valueList(node.right)
        return leftList + [node.value] + rightList
        
    def _printTree(self, node, indents):
        if node is None or node == self.NIL_LEAF:
            print("nothing")
            return
        indentStr = "  " * indents
        print(indentStr, "- ", node, sep='')
        if node.left != self.NIL_LEAF:
            if node.left.parent != node:
                print("Left parent problem here!")
            self._printTree(node.left, indents + 1)
        else:
            print(indentStr+"  " + "- Left NIL")
        if node.right != self.NIL_LEAF:
            if node.right.parent != node:
                print("Right parent problem here!")
            self._printTree(node.right, indents + 1)
        else:
            print(indentStr+"  " + "- Right NIL")
        
    def printTree(self):
        print("Tree:")
        self._printTree(self.root, 0)
        
    def isMatchingNodeInTree(self, nodeToMatch):
        def innerIsMatchingNodeInTree(rootNode, nodeToMatch):
            if rootNode is None or rootNode == self.NIL_LEAF:
                return False
            elif rootNode == nodeToMatch:
                return True
            isInLeft = innerIsMatchingNodeInTree(rootNode.left, nodeToMatch)
            isInRight = innerIsMatchingNodeInTree(rootNode.right, nodeToMatch)
            return isInLeft or isInRight
        return innerIsMatchingNodeInTree(self.root, nodeToMatch)
